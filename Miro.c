...
#define DOT_ROWS 10
#define DOT_COLS 7
#define BEEP_DURATION_US 200000 // 0.2ÃÊ
...
void HandleInput() {
    ...
    if (push_sw_buff[5] == 1 && PlayerCol < DOT_COLS - 1) { // ¡æ
        if (((Maze[PlayerRow] >> (6 - (PlayerCol + 1))) & 1) == 0) {
            PlayerCol++;
            moved = 1;
        }
    }
    else if (push_sw_buff[3] == 1 && PlayerCol > 0) { // ¡ç
        if (((Maze[PlayerRow] >> (6 - (PlayerCol - 1))) & 1) == 0) {
            PlayerCol--;
            moved = 1;
        }
    }
    else if (push_sw_buff[1] == 1 && PlayerRow > 0) { // ¡è
        if (((Maze[PlayerRow - 1] >> (6 - PlayerCol)) & 1) == 0) {
            PlayerRow--;
            moved = 1;
        }
    }
    else if (push_sw_buff[7] == 1 && PlayerRow < DOT_ROWS - 1) { // ¡é
        if (((Maze[PlayerRow + 1] >> (6 - PlayerCol)) & 1) == 0) {
            PlayerRow++;
            moved = 1;
        }
